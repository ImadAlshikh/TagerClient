...
export default function NewPostPage() {
  ...
  const createCost = prices?.find(
    (price) => price.key === "POST_CREATE",
  )?.credits;
        ...
        >
                ...
                {imagePreview ? (
                  ...
                  <div className="p-4 flex flex-col items-center gap-2 text-gray-400">
                    ...
                    <div>
                      ...
                      <br />
                      <span className="text-xs">{t("drag-and-drop")}</span>
                    </div>
                  </div>
                )}
              </div>
                ...
                >
                  {POST_CATEGORIES.map(({ value }) => (
                    <option key={value} value={value} disabled={value === ""}>
                      {value === ""
                        ? tCat("select-category")
                        : tCat(value.replace("_", "-"))}
                    </option>
                  ))}
                </select>
                {getFieldError("categoryName") && (
          ...
          <div className="bg-gray-50 p-6 md:px-8 border-t border-gray-200">
            <div className="flex flex-col md:flex-row justify-between items-center gap-6">
              ...
              {/* Actions */}
              <div className="flex flex-col lg:flex-row items-center gap-4 w-full md:w-auto justify-end">
                <div className="flex items-center gap-1 text-sm font-medium text-gray-600 bg-white px-3 py-1.5 rounded-full border border-gray-200">
                  <span>
                    {t("post-cost")} {createCost}
                  </span>
                  <img
                    src="/coin.png"
                    className="size-4"
                ...
                {promoted && (
                  <div className="flex items-center gap-1  min-w-fit text-sm font-medium text-primary bg-primary/10 px-3 py-1.5 rounded-full border border-primary/30">
                    <span>
                      {t("promote-cost")} {promoteCost}
                    </span>
                    <img
                      src="/coin.png"
                      className="size-4"
                ...
                <div className="flex">
                  ...
                  >
                    {loading ? t("posting") : t("create-post")}
                  </button>
                </div>
              </div>
            </div>
